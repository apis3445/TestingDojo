---
name: api-restsharp
description: Automate an API test case into a TUnit + RestSharp test under APINet/RestSharp/Tests/. Use when asked to automate, implement, or generate a RestSharp API test, e.g. "add a RestSharp test for test case 456" or "add a RestSharp test for DELETE /api/Server/{id}".
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, Skill, mcp__azure-devops__wit_get_work_item
---

Turn one API scenario into a TUnit + RestSharp test method in `APINet/RestSharp/Tests/`. Invocable directly, or by `api-tester` with an already-resolved scenario.
...
`Glob`/`Grep` `APINet/RestSharp/Tests/*.cs` for a class matching the resource (`{Resource}Tests.cs`) and any method already covering this endpoint + condition, so you extend an existing class/test rather than duplicate one. If `api-tester` is also generating the RestAssured version of this same scenario, use the **identical** class/method name it uses there.
...
```
RestSharp: ServerTests.cs (exists)
...
Add the method to the existing `{Resource}Tests.cs`, or create it (extending `TestBase`, namespace `APINet.RestSharp`) mirroring `LoginTests.cs` / `DashboardTests.cs` / `ServerTests.cs` — `ApiClient.Create(Configuration, AuthToken, useAuthUrl: ...)` then `GetAsync<T>`/`PostAsync<T>`/`PutAsync<T>`/`DeleteAsync`, asserting on `response.StatusCode` / `response.Data`. Add any new Model/data-factory file the plan called for.
