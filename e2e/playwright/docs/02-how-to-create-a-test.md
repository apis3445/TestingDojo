...
- **`stepDescription`**: This is the human-readable text that will appear in the final Playwright HTML report. It explains *what* the automation is doing (e.g., "Fill Password: ******").
- **`{ box: true }`**: This is a special Playwright setting that "boxes" the step in the report. If something goes wrong inside this step, the report will highlight the entire box, making it much easier to find errors at a glance.
...
### Why use POM?
- **Reusability**: If the "Login" button changes its name, you only have to update it in *one* place (the LoginPage class) instead of in every single test.
